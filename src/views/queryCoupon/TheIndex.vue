...
import { useQueryCouponStore } from '@/stores/queryCouponStore'
const searchStore = useQueryCouponStore()
...
const tableLabels = computed(() => ({
  serial_number: t('pages.queryCoupon.serial_number'),
  status: t('pages.queryCoupon.status'),
}))

function goToDetail(serial_number) {
  router.push(`/query-coupon/${serial_number}`)
}
</script>
            ...
            <tbody>
              <tr v-for="(item, index) in result_list" :key="index" class="pointer"
                ...
                <th scope="row" class="text-end">
                  ...
                  <span v-else-if="item.payment_status == 0 || item.payment_status == 1">
                    <!-- 未繳費，顯示審核與繳費狀態 -->
                    {{
                      getStatusText('form_status', item.form_status) +
                      ' / ' +

...
import { useQueryFacultyStudentParkingStore } from '@/stores/queryFacultyStudentParkingStore'
const searchStore = useQueryFacultyStudentParkingStore()
...
async function search() {
  ...
  if (search_form_request_data.value.applicant_number != 'A09035') {
...
const tableLabels = computed(() => ({
  serial_number: t('pages.queryFacultyStudentParking.serial_number'),
  status: t('pages.queryFacultyStudentParking.status'),
}))

function goToDetail(serial_number) {
  router.push(`/query-faculty-student-parking/${serial_number}`)
}
</script>
        ...
        <form>
          ...
          <div class="text-center">
            <button class="btn btn-secondary w-100" @click.prevent="search">
              {{ $t('pages.queryFacultyStudentParking.query') }}
            </button>
          </div>
        </form>

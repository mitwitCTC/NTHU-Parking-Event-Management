...
async function apply() {
  ...
  try {
    ...
    if (response.status === 200) {
      // 文件上傳成功
      router.push('/apply-event-coupon/step3')
    } else {
  ...
  <form class="mt-5">
    <div class="mb-3">
      ...
      >
        <p class="fs-4 fw-bold px-1 py-3 align-content-center m-0">
          {{
            file?.name ||
            $t('pages.applyEventCoupon.upload.upload') + (index + 1)
          }}
        </p>
    ...
    <div class="mb-3">
      ...
      >
        ...
        {{
          $t(
            'pages.applyEventCoupon.upload.certificateApplicationInstructionsRead',
          )
        }}
      </button>
    </div>
      ...
      <div class="modal-content">
        <div class="modal-header bg-secondary">
          <h5 class="modal-title text-black" id="introductionModalModalLabel">
            {{
              $t(
                'pages.applyEventCoupon.upload.certificateApplicationInstructionsTitle',
              )
            }}
          </h5>
          <button
        ...
        <div class="modal-footer">
          ...
          <p class="pointer text-primary fw-bold" @click="confirmAction">
            {{ $t('pages.applyEventCoupon.upload.confirm') }}
          </p>
        </div>
      </div>
    </div>
